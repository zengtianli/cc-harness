...
- `HANDOFF.md` — 本轮做了啥、下轮从哪开始、已知遗留问题、到期提醒（用 `/reminder` 建 Apple 提醒）
  位置：项目根 + symlink 到 `~/Dev/HANDOFF.md`（`/handoff` 命令自动做）
- 关键新踩坑 → `/distill` 提炼到 memory 或新 skill
- 本轮新建的可复用工具 → 写 `.md` 命令文档 + 放入 `~/Dev/cc-configs/commands/`
...
| 破坏性 VPS/CF 改动 | `/cf`, `/cf-audit`, `/sites-health`, `/site-rename`, `/site-archive` |
| 部署静态站 | `/ship-site`, `/deploy` |
| 健康 + 清理 | `/health`, `/tidy` |
| repo 维护链 | `/groom`（`/pull → /audit → /promote → /ship`）|
| 提炼知识 | `/distill`（改 memory + 写新 skill）|
| 复盘 | `/recap` |
| 会话收尾 | `/handoff` |
| 查历史会话 | `/scan`, `cclog` MCP, `/cmd-stats` |
| 新建 Next.js 站 | `/next-scaffold`（TODO：还没抽） |
...
| Commands/Skills 说明书 | `~/Dev/cc-configs/commands/*.md` | `cmds.tianlizeng.cloud` |
| 架构地图 | `~/Dev/stack/projects.yaml` | `stack.tianlizeng.cloud` |
| CC 演化日志 | `~/Dev/cc-evolution/changes.yaml` | `changelog.tianlizeng.cloud` |
| 会话历史 | `~/.claude/sessions/` | `cclog.tianlizeng.cloud` |
