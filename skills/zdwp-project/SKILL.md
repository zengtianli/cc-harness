...
- 格式: GDB, SHP, GeoJSON
- SSOT: `~/Dev/tools/hydro-qgis/lib/hydraulic/qgis_config.py` 的 `TARGET_CRS / GEO_CRS / COUNTY_SLICE_SOURCES`
- 工具：`bash ~/Dev/tools/hydro-qgis/scripts/cut_county.sh <县名> <输出目录>`（按县切 5 类要素）
- 县级出图：`bash ~/Dev/Work/resources/gis/bin/build_county_map.sh <市> <县>` → `output/maps/<县>_河流水系图.png`

### PyQGIS 写代码（QGIS 4.0.2 时代）

公司装的是 `QGIS-final-4_0_2.app`（PyQt 6.10.2 + Qt 6.9.3）。写 PyQGIS 代码必避：

1. **Qt 顶层 enum 永远 fully-qualified** — `Qt.AlignmentFlag.AlignHCenter`，不写 `Qt.AlignHCenter`（PyQt6 必死）
2. **共享 enum 走 Qgis namespace** — `Qgis.LabelPlacement.OverPoint`，不写 `QgsPalLayerSettings.OverPoint`（同名地雷：name 在但赋 `placement` 字段挂 TypeError）
3. **import 走 qgis.PyQt shim** — `from qgis.PyQt.QtCore import Qt`，不直接 PyQt5/PyQt6
4. **不确定的 enum 必实测**：`/Applications/QGIS-final-4_0_2.app/Contents/MacOS/python -c "from qgis.core import X; X.foo"` —— 4.0 官方 break list 不存在，agent 报告 + 网搜结果都不可信

完整迁移表 + 实测兼容期边界 → wiki [[gis-qgis-foundations/07-qgis-4-migration]]
铁律 + Why + How → memory `feedback_qgis_4_breaking.md`
...
| `projects/risk-map/qgis` | `~/Dev/tools/hydro-qgis` |
