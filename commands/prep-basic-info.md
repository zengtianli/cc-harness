...
确定目录：
- 数据目录：`~/Work/eco-flow/{city拼音}-{county}/data/`
- 参考资料：`~/Work/eco-flow/{city拼音}-{county}/data/参考资料/`
- 格式模板：`~/Work/eco-flow/lishui-景宁/data/01_基本信息.md` ~ `06_水资源概况.md`

### 2. 先查DB（优先级最高）

**DB有的数据直接用，不要网搜。** 这是铁律。

从 `~/Work/eco-flow/data/db_water_stats.db` 查询所有可用 stat_type：
...
从 `~/Work/eco-flow/data/db_reservoirs.db` 补充水库详情：
```sql
...
原始文件（PDF/网页）下载到 `~/Work/resources/data/原始xlsx/原始网搜/{县名}/`
