...
把工作内容体系化进 `<vault>/wiki/topics/<topic>/`（默认按 cwd 找最近 `.obsidian/` parent；提炼层 `~/Obsidian/dev-vault` 走旧 `topics/` 布局）。强制 Obsidian-friendly 约定：frontmatter / `[[wikilinks]]` / 50-150 行 / 每 topic 有 `_INDEX.md` MOC。

```
/wiki new <slug> "<descrip>" [--vault <path>] [--global]
/wiki entry <topic> <slug> [--vault <path>] [--global]
/wiki verify [<topic>] [--vault <path>] [--global]
/wiki link <project-path> [--vault <path>] [--global]
/wiki rebuild [--vault <path>] [--global]
```
...
- Vault 根：由 cwd 解析（见下方「Vault 解析」节）；fallback `~/Obsidian/dev-vault/`
...
**显式覆盖**：
- `--vault <path>` 指定 vault root
- `--global` 强制写 `~/Obsidian/dev-vault`（旧的"提炼层"）
