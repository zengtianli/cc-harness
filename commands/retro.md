...
- 无参数 → 写 playbook MD 到 `~/Dev/docs/knowledge/session-retro-{YYYYMMDD}-{topic-slug}.md`
- `--stdout` → 只输出到屏幕
- `--path <path>` → 指定输出路径
...
- 默认：`~/Dev/docs/knowledge/session-retro-{YYYYMMDD}-{topic-slug}.md`
- `--stdout` → 不写盘
...
参考 `~/Dev/docs/knowledge/session-retro-20260419-r7-mega.md` —— R7 本轮的 playbook，含「核心编排」「通用 Playbook」「本次漏了什么 skill」三节。
