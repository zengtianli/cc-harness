...
- 工程数据脚本：`/Users/tianli/Dev/Work/resources/data/全省水资源基础数据/工程数据/脚本/`
  ...
  - `config.py` — 路径和县市映射配置
- 数据同步：`sync_project.py`（在 `~/Dev/Work/eco-flow/`）
...
读取 `~/Dev/Work/eco-flow/sync_config.yaml` 获取县市配置（city、stat_name、project_dir）。

### 2. 检查前置数据

检查全省数据源目录是否存在：
- `/Users/tianli/Dev/Work/resources/data/全省水资源基础数据/工程数据/原始/` 下应有：
  - `表2-1工程基本情况调查表.xlsx`
  - `表2-2河湖断面生态流量排查.xlsx`
  - `表2-3水库工程生态流量排查.xlsx`
  - `浙江省已建水库名录信息.xlsx`
...
```bash
cd /Users/tianli/Dev/Work/resources/data/全省水资源基础数据/工程数据/脚本
...
```
~/Dev/Work/eco-flow/{city拼音}-{county}/data/合并数据/
```
