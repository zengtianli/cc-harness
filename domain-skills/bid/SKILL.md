...
### 阶段 2 — 招标文件解析
```bash
python3 ~/Dev/tools/scripts/scripts/document/zbwj_parse.py ~/Work/bids/{slug}/招标文件/
```
脚本输出：
...
# 5.1 质量修复（禁用词、数据来源标注）
python3 ~/Dev/tools/scripts/scripts/document/report_quality_check.py md/ --bid --fix --output-dir md_clean/

# 5.2 结构标准化（标题/编号/评分引用/加粗/来源 6 项）
python3 ~/Dev/tools/scripts/scripts/document/bid_standardize.py md_clean/ --scoring ../招标文件/scoring.json --fix --output-dir md_fixed/

# 5.3 生成图表 PNG（charts/*_config.json）
for cfg in charts/*_config.json; do
  type=$(echo "$cfg" | grep -o 'gantt\|bar\|flow')
  python3 ~/Dev/tools/scripts/scripts/document/chart_${type}.py "$cfg" -o "charts/$(basename "$cfg" _config.json).png"
done

# 5.4 ASCII art → PNG 引用
python3 ~/Dev/tools/scripts/scripts/document/chart_insert.py md_fixed/ --config charts/insert_config.json --fix --output-dir md_final/

# 5.5 合并 + 转 docx
python3 ~/Dev/tools/scripts/scripts/document/md_merge.py md_final/ -o merged.md
python3 ~/Dev/tools/scripts/scripts/document/md_docx_template.py merged.md -o 技术标.docx
```
